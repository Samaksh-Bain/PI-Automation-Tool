...
function copyFiltersToHiddenInputs() {
    ...
    // Collect all selected countries and store them in the hidden-country input as a comma-separated string
    ...
    // Store the selected countries in a hidden input
...
function drawChart(chartData) {
    ...
    const data = {
        datasets: [{
            data: chartData,
            backgroundColor: chartData.map(d => priorityCompanies.includes(d.company) ? 'rgba(139, 0, 0, 0.95)' : 'rgba(92, 92, 92, 0.75)'),
            borderColor: chartData.map(d => priorityCompanies.includes(d.company) ? 'rgba(139, 0, 0, 0.95)' : 'rgba(92, 92, 92, 0.75)'),
            borderWidth: 1
        }]
    };
